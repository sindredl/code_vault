...
class Character:
  ...
  def do_damage(self, enemy):
    ...
    else: print "%s injures %s!" % (self.name, enemy.name)
...
class Player(Character):
  ...
  def explore(self):
    if self.state != 'normal':
      ...
      if randint(0, 1):
        self.enemy = Enemy(self)
        print "A big hairy %s jumps up from nowhere!" % self.enemy.name
        self.state = 'fight'		
  ...
  def attack(self):
    if self.state != 'fight': print "%s swings into the air, pointless." % self.name; self.tired()
    else:
      if self.do_damage(self.enemy):
        ...
        if randint(0, self.health) < 10:
          ...
          print "%s is empowered!" % self.name
      else: self.enemy_attacks()
	  
  def enemy_attacks(self):
    if self.enemy.do_damage(self): print "%s was slain by %s!!!\nR.I.P." %(self.name, self.enemy.name)
